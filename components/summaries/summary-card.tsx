...
const SummaryHeader = ({
    ...
    return (
        <div className="flex items-start gap-2 sm:gap-4">
            <FileText className="w-6 h-6 sm:w-8 sm:h-8 text-violet-400 mt-1" />
            <div className="flex-1 min-w-0">
                <h3 className="text-base xl:text-lg font-semibold text-gray-900 w-4/5">{title || formatFileName(fileUrl)}</h3>
...
const StatusBadge = ({ status }: { status: string }) => {
    return (
        <span className={cn('px-3 py-1 text-xs rounded-full capitalize',
            status === 'completed' ?
            ...
            <Card className="relative h-full">
                ...
                >
                    <div className="flex flex-col gap-3 sm:gap-4">
                        <SummaryHeader
                            fileUrl={summary.origingal_file_url}
                            title={summary.title}
